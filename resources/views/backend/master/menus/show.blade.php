...
<div class="d-flex flex-column">
    <span class="fw-bold text-muted fs-6 mb-2">Add-Ons / Tambahan</span>
    @forelse ($menu->addons as $addon)
        <div class="d-flex flex-stack border-bottom border-gray-200 py-2">
            <span class="fw-semibold text-gray-800">{{ $addon->name }}</span>
            <span class="fw-bold text-success">+ Rp {{ number_format($addon->price, 0, ',', '.') }}</span>
        </div>
    @empty
        <span class="text-muted fs-7">Tidak ada add-on.</span>
    @endforelse
</div>
