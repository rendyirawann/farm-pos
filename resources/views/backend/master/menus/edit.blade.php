...
<div class="fv-row mb-7">
    ...
    <div class="form-text mb-2">Kosongkan baris untuk menghapus add-on tsb.</div>
    <div id="addons-wrapper-edit">
        @foreach ($menu->addons as $addon)
            <div class="d-flex gap-2 mb-2 addon-row">
                <input type="text" name="addon_name[]" class="form-control form-control-sm" placeholder="Nama add-on"
                    value="{{ $addon->name }}">
                <input type="number" name="addon_price[]" class="form-control form-control-sm w-125px"
                    placeholder="Harga" min="0" value="{{ round($addon->price) }}">
                <button type="button" class="btn btn-sm btn-icon btn-light-danger addon-remove"><i
                        class="ki-outline ki-trash fs-5"></i></button>
            </div>
        @endforeach
    </div>
    <button type="button" class="btn btn-sm btn-light-primary mt-2" data-addon-add="#addons-wrapper-edit">
