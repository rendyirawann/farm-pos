...
@section('content')
    ...
    @php
        ...
        $pctOfMax  = $maxPoints > 0 ? min(100, round($balance / $maxPoints * 100)) : 0;
    @endphp
                ...
                <div class="card-body">
                    <div class="row g-5">
                        <div class="col-md-4">
                            ...
                            <div class="fs-8 text-muted mt-1">Batas maksimum: Rp {{ number_format($maxPoints, 0, ',', '.') }}</div>
                        </div>
                        <div class="col-md-4">
                ...
                <div class="card-body">
                    @unless ($purchaseEnabled)
                        <div class="alert alert-warning d-flex align-items-center mb-6">
                            ...
                            <div><span class="text-gray-700">{{ $maintenanceText }}</span></div>
                        </div>
                    @endunless
                    ...
                    @unless ($tiers['any_fits'])
                        <div class="alert alert-danger d-flex align-items-center mb-6">
                            ...
                            <div>
                                ...
                                <span class="text-gray-700">Saldo poin Anda (Rp{{ number_format($balance, 0, ',', '.') }}) sudah mendekati batas maksimum Rp{{ number_format($maxPoints, 0, ',', '.') }}. Tidak ada nominal yang muat. Pakai poin dulu, lalu top-up lagi.</span>
                            </div>
                        </div>
                    @endunless
                    ...
                    <div class="row g-5">
                        @foreach ($tiers['options'] as $opt)
                            @php
                                $recommended = $tiers['recommended'] === $opt['amount'];
                                $canBuy = $opt['fits'] && $purchaseEnabled;
                            @endphp
                            <div class="col-6 col-md-3">
                                <div class="border border-dashed rounded p-4 h-100 d-flex flex-column {{ $recommended ? 'border-success bg-light-success' : ($opt['fits'] ? 'border-gray-300' : 'border-gray-300 opacity-50') }}">
                                    ...
                                    <div class="mt-auto pt-3">
                                        @if (!$opt['fits'])
                                            <button class="btn btn-sm btn-light w-100" disabled title="Saldo akhir Rp{{ number_format($opt['resulting_balance'], 0, ',', '.') }} melebihi batas maks">
                                                Melebihi batas
                                            </button>
                                        @elseif (!$purchaseEnabled)
                                            <button class="btn btn-sm btn-light w-100" disabled>Segera Hadir</button>
                                        @else
                                            <button class="btn btn-sm btn-primary w-100 btn-topup" data-amount="{{ $opt['amount'] }}" data-points="{{ $opt['points'] }}">
                                                Top Up
                                            </button>
                                        @endif
                                    </div>
            ...
            <div class="card card-flush mb-8">
                ...
                <div class="card-body">
                    <ul class="text-gray-700 fs-6 lh-lg mb-0">
                        <li>Minimal top-up Rp{{ number_format($minDeposit, 0, ',', '.') }}. Poin bernilai Rupiah (1 poin = Rp1) dan sudah termasuk bonus tiap tier.</li>
                        <li>Saldo poin <b>maksimum Rp{{ number_format($maxPoints, 0, ',', '.') }}</b>. Top-up yang membuat saldo melebihi batas akan ditolak — sistem menyarankan nominal yang muat.</li>
