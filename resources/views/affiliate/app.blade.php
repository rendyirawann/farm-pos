...
@php
    // Item navigasi (dipakai bersama sidebar & dock). [route, label, svg-path]
    $nav = [
        ['affiliate.dashboard', 'Beranda',       'M2.25 12l8.954-8.955a1.5 1.5 0 012.12 0L22.28 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75'],
        ['affiliate.link',      'Link Referral', 'M13.19 8.688a4.5 4.5 0 011.242 7.244l-3 3a4.5 4.5 0 01-6.364-6.364l1.757-1.757m13.35-.622l1.757-1.757a4.5 4.5 0 00-6.364-6.364l-3 3a4.5 4.5 0 00-1.242 7.244'],
        ['affiliate.referrals', 'Referral Saya', 'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z'],
        ['affiliate.komisi',    'Komisi',        'M21 12a2.25 2.25 0 00-2.25-2.25H15a3 3 0 11-6 0H5.25A2.25 2.25 0 003 12m18 0v6a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 18v-6m18 0V9a2.25 2.25 0 00-2.25-2.25H5.25A2.25 2.25 0 003 9v3'],
        ['affiliate.withdraw',  'Pencairan',     'M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3'],
    ];
@endphp
...
<aside class="hidden lg:flex fixed inset-y-0 left-0 z-40 w-64 flex-col border-r border-slate-200 bg-white">
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        @foreach ($nav as [$routeName, $label, $d])
            @php($active = request()->routeIs($routeName))
            <a href="{{ route($routeName) }}"
               class="flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-sm font-semibold transition {{ $active ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-indigo-50 hover:text-indigo-700' }}">
...
<nav class="lg:hidden fixed bottom-3 left-1/2 z-50 w-[94%] max-w-lg -translate-x-1/2">
    <div class="flex items-stretch justify-around gap-1 rounded-2xl border border-slate-200 bg-white/95 px-2 py-2 shadow-2xl shadow-slate-400/30 backdrop-blur">
        @foreach ($nav as [$routeName, $label, $d])
            @php($active = request()->routeIs($routeName))
            <a href="{{ route($routeName) }}"
               class="flex flex-1 flex-col items-center gap-1 rounded-xl px-1 py-1.5 text-[11px] font-semibold transition {{ $active ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500' }}">
