...
@section('content')
    ...
    <section class="relative overflow-hidden">
        ...
        <div class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 pt-14 pb-16 lg:pt-20 lg:pb-24 grid lg:grid-cols-2 gap-12 items-center">
            <div>
                <span class="inline-flex items-center gap-2 rounded-full bg-indigo-50 text-indigo-600 px-4 py-1.5 text-sm font-bold mb-6">{{ sc('affiliate','hero_badge','⭐ PROGRAM AFFILIATE') }}</span>
                <h1 class="text-4xl sm:text-5xl lg:text-[3.4rem] font-extrabold leading-[1.1] tracking-tight text-slate-900">
                    {!! sc('affiliate','hero_judul','Bagikan Mooda,<br>Dapatkan <span class="text-indigo-600">Komisi</span><br>Setiap Bulan') !!}
                </h1>
                <p class="text-lg text-slate-500 leading-relaxed mt-6 max-w-lg">
                    {{ sc('affiliate','hero_deskripsi','Promosikan Mooda POS ke audiens Anda dan dapatkan komisi') }} <span class="font-semibold text-indigo-600">{{ $komisiText }}</span>.
                </p>
                <div class="flex flex-wrap gap-3 mt-8">
                    <a href="{{ route('affiliate.register') }}" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl px-6 py-3.5 shadow-lg shadow-indigo-600/25 transition">
                        {{ sc('affiliate','hero_tombol_utama','Daftar Gratis') }} <span class="grid place-items-center w-6 h-6 rounded-full bg-white/20">→</span>
                    </a>
                    <a href="#cara-kerja" class="inline-flex items-center gap-2 bg-white border border-slate-200 text-slate-700 hover:border-indigo-300 hover:text-indigo-700 font-semibold rounded-xl px-6 py-3.5 transition">
                        {{ sc('affiliate','hero_tombol_kedua','Lihat Cara Kerja') }} <span class="grid place-items-center w-6 h-6 rounded-full bg-indigo-50 text-indigo-600">▶</span>
                    </a>
                </div>
                <div class="flex items-center gap-3 mt-8">
                    ...
                    <span class="text-sm text-slate-500">{{ sc('affiliate','hero_join_prefix','Bergabung dengan') }} <b class="text-slate-700">{{ sc('affiliate','hero_join_jumlah','1.200+') }}</b> {{ sc('affiliate','hero_join_suffix','affiliate lainnya') }}</span>
                </div>
            </div>
            ...
            <div class="relative lg:justify-self-end w-full">
                <div class="rounded-2xl bg-white border border-slate-200 shadow-2xl shadow-indigo-200/40 overflow-hidden rotate-1">
                    <div class="flex items-center gap-1.5 px-4 py-3 border-b border-slate-100 bg-slate-50">
                        ...
                        <span class="ml-3 text-xs text-slate-400 font-medium">{{ sc('affiliate','hero_mockup_judul','Affiliate Dashboard') }}</span>
                    </div>
                    <div class="p-5">
                        <div class="grid grid-cols-3 gap-3 mb-4">
                            @foreach ([[sc('affiliate','hero_mockup_stat1_label','Total Komisi'),sc('affiliate','hero_mockup_stat1_nilai','Rp 12.450.000'),'text-indigo-600'],[sc('affiliate','hero_mockup_stat2_label','Klik'),sc('affiliate','hero_mockup_stat2_nilai','18.230'),'text-slate-800'],[sc('affiliate','hero_mockup_stat3_label','Konversi'),sc('affiliate','hero_mockup_stat3_nilai','320'),'text-emerald-600']] as [$l,$v,$c])
                        ...
                        <div class="rounded-xl bg-slate-50 p-4">
                            <div class="text-xs text-slate-400 mb-2">{{ sc('affiliate','hero_mockup_performa','Performa') }}</div>
    ...
    <section class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10">
        <div class="rounded-3xl bg-white border border-slate-100 shadow-xl shadow-slate-100 grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-slate-100 overflow-hidden">
            @foreach ([
                ['💼','bg-indigo-50 text-indigo-600',sc('affiliate','fitur1_judul_prefix','Komisi') . ' ' . $komisiText, sc('affiliate','fitur1_deskripsi','Dapatkan komisi menarik setiap bulan.')],
                ['🔗','bg-sky-50 text-sky-600',sc('affiliate','fitur2_judul','Tracking Real-time'), sc('affiliate','fitur2_deskripsi','Pantau klik, konversi, dan komisi secara real-time.')],
                ['💳','bg-emerald-50 text-emerald-600',sc('affiliate','fitur3_judul','Pembayaran Tepat Waktu'), sc('affiliate','fitur3_deskripsi','Komisi dibayarkan otomatis setiap bulan.')],
    ...
    <section id="cara-kerja" class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-16 sm:py-20">
        <h2 class="text-3xl sm:text-4xl font-extrabold text-slate-900 text-center mb-14">{{ sc('affiliate','cara_kerja_judul','Cara Kerja') }}</h2>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8 md:gap-4">
            @foreach ([
                ['1','👤',sc('affiliate','langkah1_judul','Daftar'),sc('affiliate','langkah1_deskripsi','Daftar gratis sebagai affiliate Mooda.')],
                ['2','🔗',sc('affiliate','langkah2_judul','Bagikan Link'),sc('affiliate','langkah2_deskripsi','Bagikan link affiliate Anda ke audiens.')],
                ['3','🛒',sc('affiliate','langkah3_judul','Dapatkan Konversi'),sc('affiliate','langkah3_deskripsi','Setiap kali ada yang berlangganan melalui link Anda.')],
                ['4','👛',sc('affiliate','langkah4_judul','Terima Komisi'),sc('affiliate','langkah4_deskripsi','Dapatkan komisi otomatis setiap bulan.')],
    ...
    <section class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 pb-4">
        <h2 class="text-3xl sm:text-4xl font-extrabold text-slate-900 text-center mb-12">{{ sc('affiliate','potensi_judul','Potensi Penghasilan Anda') }}</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach ([
                ['📈','bg-indigo-50',sc('affiliate','potensi1_atas','Komisi Hingga'), $komisiStat, sc('affiliate','potensi1_sub','Setiap Langganan'),'text-indigo-600'],
                ['👥','bg-emerald-50','', sc('affiliate','potensi2_angka','1.200+'), sc('affiliate','potensi2_sub','Affiliate Aktif'),'text-slate-900'],
                ['💰','bg-amber-50','', sc('affiliate','potensi3_angka','Rp 2 Miliar+'), sc('affiliate','potensi3_sub','Total Komisi Dibayarkan'),'text-indigo-600'],
    ...
    <section class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-16 sm:py-20">
        <div class="rounded-3xl bg-gradient-to-br from-indigo-600 to-indigo-500 px-8 py-12 sm:px-14 text-white flex flex-col lg:flex-row items-center gap-8">
            ...
            <div class="flex-1 text-center lg:text-left">
                <h2 class="text-3xl sm:text-4xl font-extrabold leading-tight mb-2">{!! sc('affiliate','cta_judul','Mulai Hasilkan<br class="hidden sm:block"> Bersama Mooda!') !!}</h2>
                <p class="text-indigo-100 max-w-md">{{ sc('affiliate','cta_deskripsi','Gabung sekarang dan mulai dapatkan komisi dari setiap langganan.') }}</p>
            </div>
            <a href="{{ route('affiliate.register') }}" class="inline-flex items-center gap-2 bg-white text-indigo-700 font-bold rounded-xl px-8 py-4 text-lg hover:bg-indigo-50 transition shrink-0">
                {{ sc('affiliate','cta_tombol','Daftar Affiliate Gratis') }} <span class="grid place-items-center w-7 h-7 rounded-full bg-indigo-100 text-indigo-700">→</span>
            </a>
        </div>
    </section>
@endsection
