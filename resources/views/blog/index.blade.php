...
@if ($activeCat)
    ...
    <section class="bg-gradient-to-b from-indigo-50/70 to-white border-b border-slate-100">
        <div class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-14">
            <a href="{{ route('blog.home') }}" class="inline-flex items-center gap-1 text-indigo-600 text-sm font-semibold hover:text-indigo-800 mb-4">← Semua artikel</a>
            <span class="inline-block rounded-full bg-indigo-100 text-indigo-700 px-4 py-1.5 text-sm font-bold mb-4">Kategori</span>
            <h1 class="text-3xl sm:text-5xl font-extrabold tracking-tight text-slate-900">{{ $activeCat->name }}</h1>
            <p class="text-slate-500 mt-3 text-lg">Kumpulan artikel di kategori ini.</p>
        </div>
    </section>
    <div class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-12">
        @if ($posts->count())
            ...
            @endif
        @else
            <div class="text-center py-24 text-slate-500">Belum ada artikel di kategori ini.</div>
        @endif
    </div>
@else
    ...
    <section class="relative overflow-hidden">
        ...
        <div class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 pt-14 pb-16 lg:pt-20 lg:pb-24 grid lg:grid-cols-2 gap-12 items-center">
            <div>
                <span class="inline-block rounded-full bg-indigo-50 text-indigo-600 px-4 py-1.5 text-sm font-bold mb-6">Blog Resmi Mooda</span>
                <h1 class="text-4xl sm:text-5xl lg:text-[3.4rem] font-extrabold leading-[1.1] tracking-tight text-slate-900">
                    Insight &amp; Inspirasi<br>untuk <span class="text-indigo-600">Bisnis Modern</span>
                </h1>
                <p class="text-lg text-slate-500 leading-relaxed mt-6 max-w-lg">
                    Dapatkan informasi terbaru, tips praktis, dan berita menarik seputar dunia bisnis, POS, dan manajemen usaha.
                </p>
                <div class="flex flex-wrap gap-3 mt-8">
                    <a href="#trending" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl px-6 py-3.5 shadow-lg shadow-indigo-600/25 transition">
                        Jelajahi Artikel <span class="grid place-items-center w-6 h-6 rounded-full bg-white/20">→</span>
                    </a>
                    <a href="https://mooda.id" class="inline-flex items-center gap-2 bg-white border border-slate-200 text-slate-700 hover:border-indigo-300 hover:text-indigo-700 font-semibold rounded-xl px-6 py-3.5 transition">
                        Tentang Mooda <span class="grid place-items-center w-6 h-6 rounded-full bg-indigo-50 text-indigo-600">→</span>
                    </a>
                </div>
            </div>
            ...
            <div class="relative">
                <div class="rounded-2xl bg-white border border-slate-200 shadow-2xl shadow-indigo-200/40 overflow-hidden rotate-1">
                    <div class="flex items-center gap-1.5 px-4 py-3 border-b border-slate-100 bg-slate-50">
                        ...
                        <span class="ml-3 text-xs text-slate-400 font-medium">blog.mooda.id</span>
                    </div>
                    <div class="p-4">
                        <div class="text-sm font-bold text-slate-800 mb-3">Artikel Terbaru</div>
    ...
    <section id="trending" class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-6 lg:py-10">
        <div class="text-xs font-bold uppercase tracking-widest text-indigo-500 mb-1">Trending Now</div>
        <div class="flex items-end justify-between gap-4 mb-8">
            <h2 class="text-2xl sm:text-3xl font-extrabold text-slate-900">Artikel Pilihan Minggu Ini</h2>
            <a href="#semua" class="hidden sm:inline-flex items-center gap-1 text-indigo-600 font-semibold text-sm hover:text-indigo-800 shrink-0">Lihat Semua Artikel →</a>
        </div>
                ...
                <a href="{{ route('blog.show', $post->slug) }}" class="group flex flex-col rounded-2xl border border-slate-200 bg-white overflow-hidden hover:-translate-y-1 hover:shadow-xl hover:shadow-indigo-100 transition duration-300">
                    ...
                    <div class="p-5 flex flex-col grow">
                        ...
                        <div class="flex items-center gap-2 text-xs text-slate-400 mt-4">
                            <span>{{ optional($post->published_at)->locale('id')->translatedFormat('d M Y') }}</span><span>·</span><span>{{ $readTime($post) }} menit baca</span>
                        </div>
                    </div>
                </a>
            @endforeach
    ...
    <section class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-10">
        <h2 class="text-2xl font-extrabold text-slate-900 mb-6">Jelajahi Berdasarkan Kategori</h2>
            ...
            @foreach ($navCategories->take(5) as $i => $c)
                <a href="{{ route('blog.category', $c->slug) }}" class="group flex items-center gap-3 rounded-2xl border border-slate-100 p-4 hover:border-indigo-200 hover:shadow-lg hover:shadow-indigo-50 transition">
                    ...
                    <div>
                        ...
                        <div class="text-xs text-slate-400 mt-0.5">{{ $c->published_count ?? 0 }} Artikel</div>
                    </div>
                </a>
            @endforeach
        </div>
    ...
    <section id="semua" class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-10 grid lg:grid-cols-2 gap-10">
        ...
        <div>
            <div class="text-xs font-bold uppercase tracking-widest text-indigo-500 mb-1">Artikel Populer</div>
            <h2 class="text-2xl font-extrabold text-slate-900 mb-6">Paling Banyak Dibaca</h2>
                ...
                @foreach (($popular->count() ? $popular : $trending) as $i => $post)
                    <a href="{{ route('blog.show', $post->slug) }}" class="group flex items-start gap-4 py-4">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="text-xs text-slate-400 mt-1">{{ optional($post->published_at)->locale('id')->translatedFormat('d M Y') }} · {{ $readTime($post) }} menit baca</div>
                        </div>
                    </a>
                @endforeach
            </div>
        ...
        <a href="{{ route('blog.show', $spotlight->slug) }}" class="group relative rounded-3xl overflow-hidden min-h-[340px] flex items-end {{ $spotlight->cover ? 'bg-slate-900' : 'bg-gradient-to-br from-indigo-700 to-slate-900' }}">
            ...
            <div class="relative p-7 sm:p-9 text-white">
                ...
                <div class="flex items-center gap-3 text-sm">
                    <span class="text-slate-300">{{ optional($spotlight->published_at)->locale('id')->translatedFormat('d M Y') }} · {{ $readTime($spotlight) }} menit baca</span>
                    <span class="ml-auto inline-flex items-center gap-2 bg-indigo-600 group-hover:bg-indigo-500 rounded-xl px-4 py-2 font-semibold transition">Baca Selengkapnya →</span>
                </div>
            </div>
        </a>
        @endif
    ...
    <section class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-10">
        <h2 class="text-2xl font-extrabold text-slate-900 mb-6">Semua Artikel</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @foreach ($all as $post)
                @include('blog._card', ['post' => $post])
            @endforeach
    ...
    <section class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-10">
        <div class="rounded-3xl bg-indigo-50 p-8 sm:p-12 flex flex-col lg:flex-row items-center gap-8">
            ...
            <div class="flex-1 text-center lg:text-left">
                <h2 class="text-2xl font-extrabold text-indigo-700 mb-1">Dapatkan Artikel &amp; Tips Terbaru</h2>
                <p class="text-slate-500">Berlangganan newsletter kami dan dapatkan insight terbaru langsung ke email Anda setiap minggunya.</p>
            </div>
            <form onsubmit="event.preventDefault(); this.querySelector('button').textContent='Terima kasih!';" class="flex w-full lg:w-auto gap-2">
                <input type="email" required placeholder="Masukkan email Anda" class="flex-1 lg:w-64 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300">
                <button class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl px-6 py-3 text-sm transition whitespace-nowrap">Langganan</button>
            </form>
        </div>
    </section>
    @else
        <div class="text-center py-24">
            ...
            <h2 class="text-xl font-bold text-slate-900 mb-1">Belum ada artikel</h2>
