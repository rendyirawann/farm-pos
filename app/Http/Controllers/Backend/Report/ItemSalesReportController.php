...
class ItemSalesReportController extends Controller
{
    ...
    public function getData(Request $request)
    {
        if ($request->ajax()) {
            ...
            // Hitung total keseluruhan (Summary) via agregasi SQL atas subquery yang sudah di-group
            // (sebelumnya seluruh baris di-fetch ke PHP lalu di-sum)
            $totals = DB::query()->fromSub(clone $query, 'sub')
                ->selectRaw('COALESCE(SUM(total_qty), 0) as total_qty, COALESCE(SUM(total_revenue), 0) as total_revenue')
                ->first();
            $totalItemsSold = $totals->total_qty;
