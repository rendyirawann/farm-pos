...
class BillingController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        try {
            $subscription = DB::transaction(function () use ($tenant, $planKey, $amount, $months, $cashbackPercent, $cashbackAmount) {
                $orderId = 'FRM-SUB-' . strtoupper(Str::random(6)) . '-' . $tenant->id . '-' . substr((string) Str::uuid(), 0, 8);
     ...
     * Route publik, dikecualikan CSRF. Menangani langganan (FRM-SUB-) & deposit (FRM-DEP-).
     */
     ...
     * (HMAC-SHA256 raw body dgn private key), lalu aktivasi langganan (FRM-SUB-) / deposit (FRM-DEP-).
