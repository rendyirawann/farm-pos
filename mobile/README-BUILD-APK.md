# Membangun Aplikasi Tablet Mooda (.apk)

Aplikasi tablet Mooda adalah **pembungkus (wrapper)** yang menampilkan sistem web
Mooda secara layar penuh. Aplikasi **tetap butuh internet** dan **terhubung ke server**
...
Sistem ini sudah PWA (punya `manifest.json` + service worker + ikon Mooda), jadi bisa
langsung dibungkus jadi APK oleh PWABuilder.

1. Deploy Mooda ke domain **HTTPS** publik (mis. `https://app.mooda.id`).
...
6. Rename `app-release-signed.apk` → **`mooda-pos.apk`** dan taruh di
   **`public/downloads/mooda-pos.apk`** pada server. Selesai — tombol Download di
...
3. Set URL server di `app/src/main/res/values/strings.xml` → `server_url`
   (contoh `https://app.mooda.id`). Untuk server HTTP lokal saat uji coba, lihat
...
6. Rename hasilnya jadi **`mooda-pos.apk`** → taruh di **`public/downloads/`**.
...
- Icon & splash sudah tersedia di `public/assets/media/logos/` (mooda-mark-*.png,
